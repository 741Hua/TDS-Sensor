...
# 1. Introduction
This document contains the hardware bring‑up test results for the All-In-One STM32‑based Total Dissolved Solids (TDS) sensor PCB. It outlines the pass and fails per result, along with observations logged in the notes section. Failures are documented in the notes section of every test stage with a reference to the corresponding Root Cause Analysis and solution.
...
Each stage includes structured results with defined success and failure notes.
...
| Step # | Description | Success Criteria | P/F | P/F<sup>2</sup> |
|--------|-------------|------------------|-----|---|
| 1 | Connect ST‑Link SWCLK, SWDIO 3.3 V, and GND pins to the UUT on connector J2| ST‑Link connects to MCU without errors | ✅ | ✅ |
| 2 | Measure 3.3V rail with DMM at C4 | Voltage is within 3.2–3.4V | ❌<sup>1</sup> | ✅ |
| 3 | Measure 3V rail with DMM at C18 | Voltage is within 2.9–3.1V | ❌<sup>1</sup> | ✅ |
| 4 | Measure -3V rail with DMM at C20 | Voltage is within (-3.1)–(-2.9)V | ❌<sup>1</sup> | ✅ |
| 5 | Flash blink firmware (PC13 LED) | Firmware loads successfully | ✅<sup>1</sup> | ✅ |
...
## 5.2 External 12 V Power‑Up Result
...
## 5.3 RS‑485 Modbus Communication Result
